...
<div class="jumbotron jumbotron-fluid" style="margin: 0; padding: 0; text-align: center; margin-top: 70px;">
    <h1 class="lead" style="padding: 10px;">Salesman Klaten</h1>
</div>
		  ...
		  <tbody>
		  	<?php if ($nama[0]['status']=='HRD' OR $nama[0]['status']=='Owner' OR $nama[0]['status']=='Supervisor' OR $nama[0]['status']=='Oprational Manager') { ?>
			    ...
			    <tr>
			      ...
			      <td><a href="<?php echo base_url().'C_klaten/diagramsales/'.$us['Emp_Code'] ?>" class="card-link fas fa-chart-bar" style="font-size:15px;"></a></td>
			    </tr>
				<?php $no++; } ?>
		  	<?php } else {?>
			  	<?php $no=1; ?>
			    ...
			    <tr>
			      ...
			      <td><?php echo $us['Password']; ?></td>
			      <td><a href="<?php echo base_url().'C_klaten/diagramsales/'.$us['Emp_Code'] ?>" class="card-link fas fa-chart-bar" style="font-size:15px;"></a></td>
			      <td><a href="<?php echo base_url().'C_klaten/updatesales/'.$us['Emp_Code'] ?>" class="card-link fas fa-edit" style="font-size:15px;"></a></td>
			      <td><a href="<?php echo base_url().'C_klaten/hapussales/'.$us['Emp_Code'] ?>" class="card-link fas fa-trash" style="font-size:15px;" onclick="return confirm('Anda yakin ingin menghapus sales ini ?')"></a></td>
			    </tr>
				<?php $no++; } ?>
			<?php } ?>
		   </tbody>
	...
	<section class="mt-1" style="margin-bottom: 70px;">
		<a class="btn btn-dark" style="width: 100%" href="<?php echo base_url().'C_klaten/savesales' ?>">Tambah</a>
	</section>
