...
class M_klaten extends CI_Model {
	...
	public function selectsummaryklaten() {
		$hsl = $this->db->query("SELECT MonthName(Journey_Date) AS Month, YEAR(Journey_Date) AS Year, SUM(Planned) AS Planned, SUM(Un_planed) AS Un_planed, SUM(Visited) AS Visited, TIME_FORMAT(SEC_TO_TIME(avg(hour(Start_Time) * 3600 + (minute(Start_Time) * 60) + second(Start_Time))),'%H:%i:%s') as Start_Time, TIME_FORMAT(SEC_TO_TIME(avg(hour(End_Time) * 3600 + (minute(End_Time) * 60) + second(End_Time))),'%H:%i:%s') as End_Time, SUM(Nosale) as Nosale, AVG(((Visited-Un_planed)/Planned)*100) as pjp_comply, AVG((Nosale/Visited)*100) as NosalePersen, AVG(((Productive)/(Planned+Un_planed))*100) AS Productive_Call, SUM(Total_Sale) as Total_Sale FROM m_efos WHERE year(Journey_Date)=year(CURDATE()) AND id_conces = 4 GROUP BY MonthName(Journey_Date) ORDER BY Month(Journey_Date) DESC LIMIT 5");
 		return $hsl->result_array();
	}
