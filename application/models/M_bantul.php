...
class M_bantul extends CI_Model {
	...
	public function selectplanebantul() {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month,CASE DAYOFWEEK(Journey_Date)
                ...
                WHEN 6 THEN 'Jumat'
                WHEN 7 THEN 'Sabtu'
          END as Day, SUM(Planned) AS Planned, SUM(Productive) As Productive, SUM(Nosale) AS Nosale FROM m_efos WHERE id_conces = 3 AND WEEK(Journey_Date,1) = (SELECT WEEK(max(Journey_Date),1) FROM m_efos) AND YEAR(Journey_Date) = YEAR(CURDATE()) GROUP BY DAYNAME(Journey_Date) ORDER BY DAYOFWEEK(Journey_Date)");
 			return $hsl->result_array();
	}
	...
	public function selectplanebantulwk($th,$wk)	{
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month, CASE DAYOFWEEK(Journey_Date)
                ...
                WHEN 6 THEN 'Jumat'
                WHEN 7 THEN 'Sabtu'
          END as Day, SUM(Planned) AS Planned, SUM(Productive) As Productive, SUM(Nosale) AS Nosale FROM m_efos WHERE id_conces = 3 AND YEAR(Journey_Date ) = $th AND WEEK(Journey_Date,1) = $wk GROUP BY DAYNAME(Journey_Date) ORDER BY DAYOFWEEK(Journey_Date)");
 			return $hsl->result_array();
	}
	...
	public function selecttimebantulwk($th,$wk)	{
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month, CASE DAYOFWEEK(Journey_Date)
                ...
                WHEN 6 THEN 'Jumat'
                WHEN 7 THEN 'Sabtu'
          END as Day, avg(TIME_TO_SEC(End_Time-Start_Time)) as TimeInMarket, avg(TIME_TO_SEC(Spent)) as Spent, avg(TIME_TO_SEC(Time_Per_Outlet)) as TimePerOutlet FROM m_efos WHERE id_conces = 3 AND YEAR(Journey_Date) = $th AND WEEK(Journey_Date,1) = $wk GROUP BY DAYNAME(Journey_Date) ORDER BY DAYOFWEEK(Journey_Date)");
 			return $hsl->result_array();
	}
	...
	public function selectpjpbantul() {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month, CASE DAYOFWEEK(Journey_Date)
                ...
                WHEN 6 THEN 'Jumat'
                WHEN 7 THEN 'Sabtu'
          END as Day, AVG(((Visited-Un_planed)/Planned)*100) AS PJP_COMPLY, AVG(((Visited-Geo_mismatch)/Visited)*100) AS GEOMATCH, AVG(((Productive)/(Planned+Un_planed))*100) AS PRODUCTIVE_CALL FROM m_efos WHERE id_conces = 3 AND WEEK(Journey_Date,1) = (SELECT WEEK(max(Journey_Date),1) FROM m_efos) AND YEAR(Journey_Date) = YEAR(CURDATE()) GROUP BY DAYNAME(Journey_Date) ORDER BY DAYOFWEEK(Journey_Date)");
 			return $hsl->result_array();
	}
	...
	public function selectpjpbantulwk($th,$wk) {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month, CASE DAYOFWEEK(Journey_Date)
                ...
                WHEN 6 THEN 'Jumat'
                WHEN 7 THEN 'Sabtu'
          END as Day, AVG(((Visited-Un_planed)/Planned)*100) AS PJP_COMPLY, AVG(((Visited-Geo_mismatch)/Visited)*100) AS GEOMATCH, AVG(((Productive)/(Planned+Un_planed))*100) AS PRODUCTIVE_CALL FROM m_efos WHERE id_conces = 3 AND YEAR(Journey_Date) = $th AND WEEK(Journey_Date,1) = $wk GROUP BY DAYNAME(Journey_Date) ORDER BY DAYOFWEEK(Journey_Date)");
 			return $hsl->result_array();
	}
	...
	public function selectsummarybantul() {
		$hsl = $this->db->query("SELECT Journey_Date, a.Emp_Code as Emp_Code, Salesman, YEAR(Journey_Date) AS Year, MONTH(Journey_Date) AS Month, WEEK(Journey_Date,1) AS Week, SUM(Planned) AS Planned, SUM(Un_planed) AS Un_planed, SUM(Visited) AS Visited, TIME_FORMAT(SEC_TO_TIME(avg(hour(Start_Time) * 3600 + (minute(Start_Time) * 60) + second(Start_Time))),'%H:%i:%s') as Start_Time, TIME_FORMAT(SEC_TO_TIME(avg(hour(End_Time) * 3600 + (minute(End_Time) * 60) + second(End_Time))),'%H:%i:%s') as End_Time, SUM(Nosale) as Nosale, AVG(((Visited-Un_planed)/Planned)*100) as pjp_comply, AVG((Nosale/Visited)*100) as NosalePersen, AVG(((Productive)/(Planned+Un_planed))*100) AS Productive_Call, SUM(Total_Sale) as Total_Sale FROM m_efos a LEFT JOIN m_selesman b ON a.Emp_Code = b.Emp_Code WHERE WEEK(Journey_Date,1) = (SELECT WEEK(max(Journey_Date),1) FROM m_efos) AND YEAR(Journey_Date) = YEAR(CURDATE()) AND a.id_conces = 3 GROUP BY a.Emp_Code ORDER BY Productive_Call");
 		return $hsl->result_array();
	}

	public function selectsummarybantulwk($th,$wk) {
		$hsl = $this->db->query("SELECT Journey_Date, a.Emp_Code as Emp_Code, Salesman, YEAR(Journey_Date) AS Year, MONTH(Journey_Date) AS Month, WEEK(Journey_Date,1) AS Week, SUM(Planned) AS Planned, SUM(Un_planed) AS Un_planed, SUM(Visited) AS Visited, TIME_FORMAT(SEC_TO_TIME(avg(hour(Start_Time) * 3600 + (minute(Start_Time) * 60) + second(Start_Time))),'%H:%i:%s') as Start_Time, TIME_FORMAT(SEC_TO_TIME(avg(hour(End_Time) * 3600 + (minute(End_Time) * 60) + second(End_Time))),'%H:%i:%s') as End_Time, SUM(Nosale) as Nosale, AVG(((Visited-Un_planed)/Planned)*100) as pjp_comply, AVG((Nosale/Visited)*100) as NosalePersen, AVG(((Productive)/(Planned+Un_planed))*100) AS Productive_Call, SUM(Total_Sale) as Total_Sale FROM m_efos a LEFT JOIN m_selesman b ON a.Emp_Code = b.Emp_Code WHERE WEEK(Journey_Date,1) = '$wk' AND year(Journey_Date) = '$th' AND a.id_conces = 3 GROUP BY a.Emp_Code ORDER BY a.Emp_Code ORDER BY Productive_Call");
 		return $hsl->result_array();
	}

	public function selectsummarybantulbln($jd) {
		$hsl = $this->db->query("SELECT Journey_Date, a.Emp_Code as Emp_Code, Salesman, YEAR(Journey_Date) AS Year, MONTHNAME(Journey_Date) AS Month, SUM(Planned) AS Planned, SUM(Un_planed) AS Un_planed, SUM(Visited) AS Visited, TIME_FORMAT(SEC_TO_TIME(avg(hour(Start_Time) * 3600 + (minute(Start_Time) * 60) + second(Start_Time))),'%H:%i:%s') as Start_Time, TIME_FORMAT(SEC_TO_TIME(avg(hour(End_Time) * 3600 + (minute(End_Time) * 60) + second(End_Time))),'%H:%i:%s') as End_Time, SUM(Nosale) as Nosale, AVG(((Visited-Un_planed)/Planned)*100) as pjp_comply, AVG((Nosale/Visited)*100) as NosalePersen, AVG(((Productive)/(Planned+Un_planed))*100) AS Productive_Call, SUM(Total_Sale) as Total_Sale FROM m_efos a LEFT JOIN m_selesman b ON a.Emp_Code = b.Emp_Code WHERE MONTH(Journey_Date) = MONTH('$jd') AND year(Journey_Date) = YEAR('$jd') AND a.id_conces = 3 GROUP BY a.Emp_Code ORDER BY Productive_Call");
 		return $hsl->result_array();
	}
