...
class M_bantul extends CI_Model {
	...
	public function selectplanebantul() {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month, DAYNAME(Journey_Date) AS Day, SUM(Planned) AS Planned, SUM(Productive) As Productive, SUM(Nosale) AS Nosale FROM m_efos WHERE id_conces = 3 AND WEEK(Journey_Date,1) = WEEK(CURDATE()) AND YEAR(Journey_Date) = YEAR(CURDATE()) GROUP BY DAYNAME(Journey_Date)");
 			return $hsl->result_array();
	}

	public function selectplanebantulwk($th,$wk)	{
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month, DAYNAME(Journey_Date) AS Day, SUM(Planned) AS Planned, SUM(Productive) As Productive, SUM(Nosale) AS Nosale FROM m_efos WHERE id_conces = 3 AND YEAR(Journey_Date ) = $th AND WEEK(Journey_Date,1) = $wk GROUP BY DAYNAME(Journey_Date)");
 			return $hsl->result_array();
	}
	...
	public function selectpjpbantul() {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month, DAYNAME(Journey_Date) AS Day, AVG(((Visited-Un_planed)/Planned)*100) AS PJP_COMPLY, AVG(((Visited-Geo_mismatch)/Visited)*100) AS GEOMATCH, AVG(((Productive)/(Planned+Un_planed))*100) AS PRODUCTIVE_CALL FROM m_efos WHERE id_conces = 3 AND WEEK(Journey_Date,1) = WEEK(CURDATE()) AND YEAR(Journey_Date) = YEAR(CURDATE()) GROUP BY DAYNAME(Journey_Date)");
 			return $hsl->result_array();
	}

	public function selectpjpbantulwk($th,$wk) {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as month, DAYNAME(Journey_Date) AS Day, AVG(((Visited-Un_planed)/Planned)*100) AS PJP_COMPLY, AVG(((Visited-Geo_mismatch)/Visited)*100) AS GEOMATCH, AVG(((Productive)/(Planned+Un_planed))*100) AS PRODUCTIVE_CALL FROM m_efos WHERE id_conces = 3 AND YEAR(Journey_Date) = $th AND WEEK(Journey_Date,1) = $wk GROUP BY DAYNAME(Journey_Date)");
 			return $hsl->result_array();
	}
