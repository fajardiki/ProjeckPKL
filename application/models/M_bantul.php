...
class M_bantul extends CI_Model {
	...
	public function selecttime() {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as Week, avg(TIME_TO_SEC(Time_in_Market)) as TimeInMarket, avg(TIME_TO_SEC(Spent)) as Spent, avg(TIME_TO_SEC(Time_Per_Outlet)) as TimePerOutlet FROM m_efos WHERE id_conces = 3 GROUP BY WEEK(Journey_Date,1) DESC LIMIT 5");
	...
	public function selectonetimebantul($bln, $thn) {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) AS Week, avg(TIME_TO_SEC(Time_in_Market)) as TimeInMarket, avg(TIME_TO_SEC(Spent)) as Spent, avg(TIME_TO_SEC(Time_Per_Outlet)) as TimePerOutlet FROM m_efos WHERE MONTH(Journey_Date)='$bln' AND year(Journey_Date)='$thn' AND id_conces = 3 GROUP BY WEEK(Journey_Date,1) DESC");
