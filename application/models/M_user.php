...
class M_user extends CI_Model {
	...
	// time
		...
		// Dassboar
			public function selectonetime($emp_code) {
				$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as Week, DAYNAME(Journey_Date) as Day, TIME_TO_SEC(End_Time)-TIME_TO_SEC(Start_Time) as TimeInMarket, TIME_TO_SEC(Spent) as Spent, TIME_TO_SEC(Time_Per_Outlet) as TimePerOutlet FROM m_efos WHERE Emp_Code = $emp_code AND WEEK(Journey_Date,1) = WEEK(CURDATE())-1");
			...
			public function selectonetimenow($emp_code) {
				$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as Week, DAYNAME(Journey_Date) as Day, TIME_TO_SEC(End_Time)-TIME_TO_SEC(Start_Time) as TimeInMarket, TIME_TO_SEC(Spent) as Spent, TIME_TO_SEC(Time_Per_Outlet) as TimePerOutlet FROM m_efos WHERE Emp_Code = $emp_code AND WEEK(Journey_Date,1) = WEEK(CURDATE())");
	...
	public function selecttimesales($empcode) {
		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) AS Week, avg(TIME_TO_SEC(End_Time)-TIME_TO_SEC(Start_Time)) as TimeInMarket, avg(TIME_TO_SEC(Spent)) as Spent, avg(TIME_TO_SEC(Time_Per_Outlet)) as TimePerOutlet FROM m_efos WHERE Emp_Code = '$empcode' GROUP BY WEEK(Journey_Date,1) DESC LIMIT 5");
	...
	public function selecteonefossales($bln, $thn, $empcode) {
 	...
 	public function selectsummary($empcode)	{
 		$hsl = $this->db->query("SELECT WEEK(Journey_Date,1) as Week, YEAR(Journey_Date) as Year, Journey_Date as Day, Planned, Un_planed, Visited, TIME_FORMAT(SEC_TO_TIME(avg(hour(Start_Time) * 3600 + (minute(Start_Time) * 60) + second(Start_Time))),'%H:%i:%s') as Start_Time, TIME_FORMAT(SEC_TO_TIME(avg(hour(End_Time) * 3600 + (minute(End_Time) * 60) + second(End_Time))),'%H:%i:%s') as End_Time, SUM(Nosale) as Nosale, AVG(((Visited-Un_planed)/Planned)*100) as pjp_comply, AVG((Nosale/Visited)*100) as NosalePersen, AVG(((Productive)/(Planned+Un_planed))*100) AS Productive_Call, SUM(Total_Sale) as Total_Sale FROM m_efos WHERE Emp_Code='$empcode' AND WEEK(Journey_Date,1)=WEEK(CURDATE()) GROUP BY DAY(Journey_Date) ORDER BY Journey_Date");
 		return $hsl->result_array();
 	}
